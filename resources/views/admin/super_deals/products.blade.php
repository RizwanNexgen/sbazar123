...
@section('content')
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <h1> Super Deals <small>Add Products...</small> </h1>
            <ol class="breadcrumb">
                <li><a href="{{ URL::to('admin/dashboard/this_month') }}"><i class="fa fa-dashboard"></i> {{ trans('labels.breadcrumb_dashboard') }}</a></li>
                <li><a href="{{ URL::to('admin/super-deals/display')}}"><i class="fa fa-industry"></i> Super Deals</a></li>
                                    ...
                                    <div class="box-body">
                                        <div class="form-group">
                                            ...
                                            <table id="myTable" class="table table-bordered table-striped">
                                                ...
                                                @if(count($list)>0)
                                                    @foreach ($list  as $o)
                                                    <tr id="tr_{{ $o->products_id }}">
                                                        <td>{{$o->products_id}}</td>                                                                     
                                                        <td>{{$o->products_name}} ({{$o->products_weight}} {{$o->products_weight_unit}})</td>
                                                        <td>€{{$o->products_price}}</td>  
                                                        <td>{{$o->products_sku}}</td>                                            
                                                        <td>
                                                            <a href="javascript:void(0)" onClick="addSuperDealsProduct({{$o->products_id}})" class="btn btn-success btn-sm">Add</a>                                                            
                                                        </td>
                                                    </tr>
                                                    @endforeach                                
                                                @endif
    ...
    <script type="text/javascript">
        ...
        function addSuperDealsProduct(product_id)
        {
            $.ajax({
                url:'{{url("admin/super-deals/add_product")}}',
                ...
                success:function(response){
                    if(response.error=='N') {
                        ...
                        console.log($('#tr_'+response.product.products_id));
