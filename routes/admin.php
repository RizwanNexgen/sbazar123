...
Route::get('/test', function () {
  ...
  $first = 'Rizwan';
  $last  = 'Afzal';
  $result1 = strtoupper(substr($first, 0, 2));
  $result2 = strtoupper(substr($last, 0, 2));
  $res3 = rand(10,100);
  $year =  strtoupper(substr(date("Y"), 2, 4));



   $referalID = 'S'. $result1 . $result2 .$year.$res3  ;
...
Route::get('/test1', function () {
   ...
   $email = 'Cloudways';
   Mail::to('m.rizwannexgen@gmail.com')->send(new EmailVerification($email));
   return 'success';

});

Route::get('/verify/{email}', 'WelcomeController@verify')->name('verify');
...
Route::group(['middleware' => ['installer']], function () {
    ...
    Route::group(['prefix' => 'admin/hotcats', 'middleware' => 'auth', 'namespace' => 'AdminControllers'], function () {

        Route::get('/display', 'HotController@display');

        Route::get('/add', 'HotController@add');

        Route::post('/add', 'HotController@insert');

        Route::get('/edit/{id}', 'HotController@edit');

        Route::post('/update', 'HotController@update');

        Route::post('/delete', 'HotController@delete');

        Route::get('/filter', 'HotController@filter');

    });
    
    Route::group(['prefix' => 'admin/memberships', 'middleware' => 'auth', 'namespace' => 'AdminControllers'], function () {

        Route::get('/display', 'MembershipController@display');

        Route::get('/add', 'MembershipController@add');

        Route::post('/add', 'MembershipController@insert');

        Route::get('/edit/{id}', 'MembershipController@edit');

        Route::post('/update', 'MembershipController@update');

        Route::post('/delete', 'MembershipController@delete');

        Route::get('/filter', 'MembershipController@filter');
    ...
    Route::group(['prefix' => 'admin/products', 'middleware' => 'auth', 'namespace' => 'AdminControllers'], function () {
        ...
        Route::post('/get_single_product/{id}', 'ProductController@get_single_product')->name('get_single_product');
        ...
        Route::get('/insert_ajax', 'ProductController@insert_ajax');

        Route::post('/delete_ajax', 'ProductController@delete_ajax');

        Route::get('/filter', 'ProductController@filter')->middleware('view_product');

        Route::get('/filter_product', 'ProductController@filter_product')->middleware('view_product');
...
//customers
    ...
    Route::group(['prefix' => 'admin/shippingmethods', 'middleware' => 'auth', 'namespace' => 'AdminControllers'], function () {
        ...
        Route::get('/shippingDhl', 'ShippingMethodsController@shippingDhl')->middleware('view_shipping');

        Route::post('/updateShippingDhl', 'ShippingMethodsController@updateShippingDhl')->middleware('edit_shipping');

        Route::get('/shippingDhlExpress', 'ShippingMethodsController@shippingDhlExpress')->middleware('view_shipping');

        Route::post('/updateShippingDhlExpress', 'ShippingMethodsController@updateShippingDhlExpress')->middleware('edit_shipping');
        
        Route::get('/shippingDpd', 'ShippingMethodsController@shippingDpd')->middleware('view_shipping');
        Route::post('/updateShippingDpd', 'ShippingMethodsController@updateShippingDpd')->middleware('edit_shipping');
        Route::get('/dpdCountries', 'ShippingMethodsController@dpdCountries')->middleware('view_shipping');
        Route::post('/updateDpdCountries', 'ShippingMethodsController@updateDpdCountries')->middleware('edit_shipping');
        Route::get('/dpdCountries/delete/{id}', 'ShippingMethodsController@dpdCountries_delete')->middleware('edit_shipping');
        
        Route::get('/shippingHermes', 'ShippingMethodsController@shippingHermes')->middleware('view_shipping');
        Route::post('/updateShippingHermes', 'ShippingMethodsController@updateShippingHermes')->middleware('edit_shipping');
        Route::get('/hermesCountries', 'ShippingMethodsController@hermesCountries')->middleware('view_shipping');
        Route::post('/updateHermesCountries', 'ShippingMethodsController@updateHermesCountries')->middleware('edit_shipping');
        Route::get('/hermesCountries/delete/{id}', 'ShippingMethodsController@hermesCountries_delete')->middleware('edit_shipping');
    ...
    Route::group(['prefix' => 'admin', 'middleware' => 'auth', 'namespace' => 'AdminControllers'], function () {
        ...
        Route::get('bundles/display', 'BundleController@display');        
        Route::match(['get', 'post'], 'bundles/add', 'BundleController@add'); 
        Route::match(['get', 'post'], 'bundles/edit/{id}', 'BundleController@edit');                
        Route::get('bundles/delete/{id}', 'BundleController@delete');
        Route::get('bundles/filter', 'BundleController@filter');
        Route::post('bundles/update_bundle_quantity', 'BundleController@update_bundle_quantity');
        Route::get('bundles/delete_bundle_product/{id}', 'BundleController@delete_bundle_product');
        
        Route::get('faq/display', 'FaqController@display');        
        ...
        Route::get('faq-cat/display', 'FaqCatController@display');        
        Route::match(['get', 'post'], 'faq-cat/add', 'FaqCatController@add'); 
        Route::match(['get', 'post'], 'faq-cat/edit/{id}', 'FaqCatController@edit');                
        Route::get('faq-cat/delete/{id}', 'FaqCatController@delete');
        
        /*For Points Setting*/
        Route::match(['get', 'post'], 'points_setting/display', function(){
            $title = array('pageTitle' => Lang::get("labels.setting"));

            $result = array();
            $st = new App\Http\Controllers\AdminControllers\SiteSettingController();
            $settings = $st->Setting->getallsetting();
    
            $result['settings'] = $settings->unique('id')->keyBy('id');
            $result['commonContent'] = $st->Setting->commonContent();
    
            return view("admin.settings.general.points_setting", $title)->with('result', $result);
        }); 
        /*For Points Setting*/
        
        /*For Combo Routes*/
        Route::get('combos/display', 'ComboController@display');
        Route::match(['get', 'post'], 'combos/add', 'ComboController@add'); 
        Route::match(['get', 'post'], 'combos/edit/{id}', 'ComboController@edit');                
        Route::get('combos/delete/{id}', 'ComboController@delete');
        Route::get('combos/filter', 'ComboController@filter');
        Route::post('combos/update_combo_quantity', 'ComboController@update_combo_quantity');
        Route::get('combos/delete_combo_product/{id}', 'ComboController@delete_combo_product');
        /*For Combo Routes*/
        
        /*For Super Deals Routes*/
        Route::get('super-deals/display', 'SuperDealsController@display');
        Route::match(['get', 'post'], 'update-super-deals', 'SuperDealsController@update_super_deals');
        Route::get('super-deals/products', 'SuperDealsController@products');  
        Route::get('super-deals/search_products', 'SuperDealsController@search_products');
        Route::get('super-deals/add_product', 'SuperDealsController@add_product');  
        Route::get('super-deals/remove_products/{product_id}', 'SuperDealsController@remove_product');
        /*For Super Deals Routes*/
        
        /*For Flash Deals Routes*/
        Route::get('flash-deals/display', 'FlashDealsController@display');
        Route::match(['get', 'post'], 'update-flash-deals', 'FlashDealsController@update_flash_deals');
        Route::get('flash-deals/products', 'FlashDealsController@products');  
        Route::get('flash-deals/search_products', 'FlashDealsController@search_products');
        Route::get('flash-deals/add_product', 'FlashDealsController@add_product');  
        Route::get('flash-deals/remove_products/{product_id}', 'FlashDealsController@remove_product');
        /*For Flash Deals Routes*/
        
        Route::get('packages/display', 'PackageController@display');  
        Route::match(['get', 'post'], 'packages/add', 'PackageController@add'); 
        Route::match(['get', 'post'], 'packages/edit/{id}', 'PackageController@edit');                
        Route::get('packages/delete/{id}', 'PackageController@delete');
        Route::get('packages/products/{id}', 'PackageController@products');  
        Route::get('packages/search_products', 'PackageController@search_products');
        Route::get('packages/add_product', 'PackageController@add_product');  
        Route::get('packages/remove_products/{product_id}/{package_id}', 'PackageController@remove_product');
        
        Route::get('newsletters/display', 'NewsletterController@display');

        Route::match(['get', 'post'], 'newsletters/add', 'NewsletterController@add');  

        Route::match(['get', 'post'], 'newsletters/edit/{id}', 'NewsletterController@edit');         

        Route::get('newsletters/delete/{id}', 'NewsletterController@delete');

        Route::match(['get', 'post'], 'newsletters/send/{id}', 'NewsletterController@send'); 
        ...
        Route::get('appmenus/delete/{id}', 'AppMenuController@delete');
