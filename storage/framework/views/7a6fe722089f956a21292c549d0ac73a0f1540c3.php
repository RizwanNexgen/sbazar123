...
<script src="<?php echo asset('admin/plugins/datetimepicker/bootstrap-datetimepicker.js'); ?>"></script>
...
$(document).on('click', '#updateAddress', function(e){
	...
	//deleteHotcatModal
	$(document).on('click', '#hotcatFrom', function(){
		var hotcat_id = $(this).attr('hotcats_id');
		$('#hotcats_id').val(hotcat_id);
		$("#hotcatModal").modal('show');
	});
	
	//deleteMembershipModal
	$(document).on('click', '#MembershipFrom', function(){
		var membership_id = $(this).attr('membership_id');
		$('#del_membership_id').val(membership_id);
...
$(document).on('keyup focusout', '.phone-validate', function(e){
	...
	$(document).on('keyup focusout', '.flash-price-validate', function(e){
		var products_price = $('#products_price').val();
		console.log('Hi '+products_price+' '+this.value);
		if(this.value == ''  || this.value < 0 || isNaN(this.value)) {
			$(this).closest(".form-group").addClass('has-error');
			error = "has error";
...
$(document).on('click','#selectedICONE2', function(){
    var image_src = $('.thumbnail.selected').children('img').attr('src');
	if(image_src != undefined){
		$('#selectedthumbnailIcon2').html('<img src="'+image_src+'" class = "thumbnail" style="max-height: 100px; margin-top: 20px; ">');
		$('#selectedthumbnailIcon2').show();
		$('#image-Icone2').show();
		$('#imageIcone2').removeClass('has-error');
		$('#newIcon2').removeClass('field-validate');
	}
});
...
<?php /**PATH /home/pidesqfm/square.pidetutransporte.com/resources/views/admin/common/scripts.blade.php ENDPATH**/ ?>