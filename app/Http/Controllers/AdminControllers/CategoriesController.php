...
class CategoriesController extends Controller
{
  ...
  public function insert(Request $request){
        ...
        if($request->hasFile('cat_img')){
            $file_name = date('m-d-Y_H:i:s') . '_'. $request->file('cat_img')->getClientOriginalName();
            $source = \Tinify\fromFile($request->file('cat_img'));
            $this->s3_credential['path'] = env('S3_BUCKET_NAME'). '/category-images'. '/' . $file_name;
            $source->store($this->s3_credential);
            $uploadImage = 'http://'.env('S3_BUCKET_NAME') . '.s3.' . env('S3_REGION') . '.amazonaws.com/category-images/' . $file_name;
        }

        if($request->hasFile('cat_icon')){
            $file_name = date('m-d-Y_H:i:s') . '_'. $request->file('cat_icon')->getClientOriginalName();
            $source = \Tinify\fromFile($request->file('cat_icon'));
            $this->s3_credential['path'] = env('S3_BUCKET_NAME'). '/category-images'. '/' . $file_name;
            $source->store($this->s3_credential);
            $uploadIcon = 'http://'.env('S3_BUCKET_NAME') . '.s3.' . env('S3_REGION') . '.amazonaws.com/category-images/' . $file_name;
        }
