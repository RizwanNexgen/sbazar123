...
// slugify method
    ...
    public function updateSetting(Request $request)
    {
        ...
        foreach ($request->all() as $key => $value) {
            ...
            elseif ($key == 'search_tags') {
                
                if( $request->search_tags !== null){
            ...
            elseif ($key == 'welcome_background') 
            {
                if($request->hasFile('welcome_background')) 
                {
                    /*$file = $request->file('welcome_background');
                    $path = $file->store('public/storage/app/welcome');
                    $app_welcome_background = $file->hashName(); */
                    
                    $file = $request->file('welcome_background');
                    $fileName = $file->getClientOriginalName() ;
                    $destinationPath = public_path().'/storage/app/welcome';
                    $file->move($destinationPath,$fileName);

                    $this->Setting->settingUpdate('app_welcome_background', $fileName);      
                }  
            }
            elseif ($key == 'splash_background') 
            {
                if($request->hasFile('splash_background')) 
                {
                    /*$file = $request->file('splash_background');
                    
                    $path = $file->store('storage/app/splash');
                    $app_splash_background = $file->hashName();*/
                    
                    $file = $request->file('splash_background');
                    $fileName = $file->getClientOriginalName() ;
                    $destinationPath = public_path().'/storage/app/splash' ;
                    $file->move($destinationPath,$fileName);
                    
                    //dd($path);
                    
                    $this->Setting->settingUpdate('app_splash_background', $fileName);      
                }  
            }
            else{
