...
class Categories extends Model
{
    ...
    public function paginator(){
      ...
      $categories = Categories::sortable(['categories_id'=>'ASC'])
           ...
           /* ->LeftJoin('image_categories as categoryTable', function ($join) {
            ...
            'categories_description.language_id','categories.categories_image as imgpath','categories.categories_icon as iconpath', 
            ...
            //->where('categories.categories_id', 8536)
            ...
            /*foreach($categories as $cat){
                ...
                if($cat->parent_id != 0){
                    ...
                    $cat_names = '';
                    for(;;){
                        $c = DB::table('categories')->where('categories.categories_id', $cat_id)
                            ->leftJoin('categories_description','categories_description.categories_id', '=', 'categories.categories_id')
                            ->first();
                        if($c->parent_id == 0){
                            break;
                        ...
                        $cat_names .= $c->categories_name; 
                    }
                    //dd($cat_names);
                    
                    $cat->parent_name .= $cat_names;
                }
            }*/
          ...
          case 'Name':
              $categories = Categories::sortable(['categories_id'=>'ASC'])
                    ->leftJoin('categories_description','categories_description.categories_id', '=', 'categories.categories_id')
                        ...
                        'categories.categories_icon as icon',  'categories.created_at as date_added',
                        'categories.updated_at as last_modified', 'categories_description.categories_name as name',
                        'categories_description.language_id','categories.categories_image as imgpath','categories.categories_icon as iconpath','categories.categories_status  as categories_status')
                        ->where('categories_description.language_id', '1')
                        ->where(function($query) {
                            $query->where('categoryTable.image_type', '=',  'THUMBNAIL')
                                ->where('categoryTable.image_type','!=',   'THUMBNAIL')
    ...
    public function edit($request){
        $category = DB::table('categories') ->leftJoin('images','images.id', '=', 'categories.categories_image')
            ...
            'categories.categories_image as imgpath','categories.categories_icon as iconpath')
    ...
    public function updaterecord($categories_id,$uploadImage,$uploadIcon,$last_modified,$parent_id,$slug,$categories_status){
        ...
        if($uploadImage != null){
            $data['categories_image'] = $uploadImage;
        }
        if($uploadIcon != null){
            $data['categories_icon'] = $uploadIcon;
        }
    ...
    public function editsubcategory($request){
        $editSubCategory = DB::table('categories')
            ...
            'categories.categories_slug as slug', 'categories.categories_status  as categories_status', 'categories.parent_id as parent_id','categories.categories_image as imgpath','categories.categories_icon as iconpath')
            ->where('categories.categories_id', $request->id)->get();
