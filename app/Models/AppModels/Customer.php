            ...
            if (Auth::attempt($customerInfo)) {
               DB::table('users')->where('email', $email)->update([
                ...
                if (count($existUser) > 0) {
                    ...
                    $responseData = array('success' => '1', 'data' => $existUser, 'message' => 'Logged In Successfully!');
    ...
    public static function processregistration($request)
    {
       ...
       if($request->ref_id){
        ...
        if ($authenticate == 1 or 1==1) {
            ...
            if(count($errors)>0){
                $responseData = array('success' => '0', 'data' => $validator->messages(), 'message' => "Some paramters are missing.");
            }else{
            ...
            if (count($existUser) > 0) {
                ...
                $responseData = array('success' => '0', 'data' => $postData, 'message' => "Email address is already exist");
                  ...
                  $result1 = strtoupper(substr($customers_firstname, 0, 2));
                  $result2 = strtoupper(substr($customers_lastname, 0, 2));
                  $res3 = rand(10,100);
                  $year =  strtoupper(substr(date("Y"), 2, 4));
                  $referalID = 'SB'. $result1 . $result2 .$year. $res3  ;
